...
def create_format(cTw,wTc,cTbase,wTbase,w_Eu0,w_Eu1,w_Eu2,w_pix,w_angle0,w_angle1,w_angle2,w_angle3,w_angle4,w_angle5,finc,cTq,qTc,c2Tbase,qTbase,q_Eu0,q_Eu1,q_Eu2,q_pix,q_angle0,q_angle1,q_angle2,q_angle3,q_angle4,q_angle5,finq1,wTq):
    ...
    for i in range(len(names)):
        mydict[names[i]] =[np.round(scores[i],3)] 
    return mydict
        ...
        if rv == True:
            ...
            for i in range(len(datas)):
                ...
                if datas[i] !='':
                    ...
                    count+=1
                    cTq0,cTq1,cTq2,fin = ipm(p0,p1,p2,width,heigh,Q12,Q23,Q13,z,Zs[datas[i]][0],Zs[datas[i]][1],Zs[datas[i]][2])
                    ...
                    cTq_[datas[i]] = [cTq0,cTq1,cTq2,p1,p2,fin,np.round(d,3)]
                    #p1、p2轉換成wTp1、wTp2
                    #wTp1 = wTc * p1
            ...
            for i in range(len(is_find)):
                if is_find[names[i]]==True:
                    if names[i]==names[0]:
                        ...
                        print('QR0: ',' pixel:', pix,' fin:', np.round(fin,6))
                        finc = fin
                        ...
                        if len(wTc)!=0:
                            ...
                            print('QR',i,':' , ' pixel:', pix,' fin:', np.round(fin,6))
                            if i ==1:
                                wTq1 = wTqn
                                finq1 =fin
                            if i ==2:
